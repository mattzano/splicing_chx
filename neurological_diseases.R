...
input_disease <- list.files(path = here::here("data/panels"))
datalist_disease <- list()

for (i in input_disease) {
  #input_tsv <- paste(i, "tsv", sep = ".")
  input_disease_tsv <- read.table(file.path(here::here(), "data", "panels", i), sep = "\t", header = T, fill = TRUE)
...
big_cryptic_disease_labelled2 %>%
  ...
  #facet_wrap(facets = vars(Level3), scales = "free_y") +
  geom_bar(aes(fill = category), position = position_dodge(width = 0.5)) +
