...
salmonQuant <- importIsoformExpression(parentDir = "~/Desktop/salmon_chx/")
...
brain <- extractTopSwitches(exampleSwitchListAnalyzed, filterForConsequences = TRUE, n=1000)
write.csv(brain, "~/Desktop/brain.csv", row.names = F)
muscle <- read.csv("~/Desktop/muscle.csv")
brain_filter <- brain %>%
  dplyr::filter(gene_name %in% muscle$gene_name)
#for (i in brain_filter$gene_name) {
switchPlot(
  exampleSwitchListAnalyzed,
  gene= "ERCC4",
