...
middle_folder = 'modulizers/' 
summary_file = fread(file.path(top_folder,middle_folder,paste0(bottom_folder,'/junctions.tsv')))

summary_file = fread(file.path(top_folder,middle_folder,paste0(bottom_folder,'/summary.tsv')))
files_types_cryptics = summary_file %>% 
    filter(gene_id %in% all_cryptic_splicing$gene_id) %>% 
    # select(-c(module_id,seqid,strand,lsv_id,num_events)) %>% filter(gene_name == "UNC13A") %>% 
    select(gene_id,gene_name,module_event_combination) %>% 
    mutate(module_event_combination = gsub("\\^\\d+","",module_event_combination)) %>% 
    separate_rows(module_event_combination,sep = "\\|") %>% 
    unique() 
# cassette exons -----------------------------------------------------------
